...
class BasicLayout:
    ...
    def draw_blurr(self, canvas, coord_list, color=colors['white'], blur_radius=3):
        for xy in coord_list:
            xy = np.array(xy + 0.5, dtype=int)
            canvas = cv2.circle(canvas, (xy[0], xy[1]), blur_radius, color, -1)

        # Apply Gaussian blur around the points in coord_list
        canvas = cv2.GaussianBlur(canvas, (2 * blur_radius + 1, 2 * blur_radius + 1), 0)
